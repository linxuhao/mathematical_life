...
the cloud size \emph{equals} the number of reasoning steps, their topological features (H$_0$/H$_1$
counts, total persistence) grow with that count, and the alignment-score target also covaries with
length---so the reported variance is consistent with an uncontrolled length confound. The direct
...
Trajectory-topology positives in this genre \citep{reasoningTrajectories2604} likewise report no
length or difficulty control. We do not claim Tan et al.\ are wrong on their own target; we claim the
result does not transfer to the activation-manifold capacity reading our ontology requires, and that
its most likely basis---trajectory length---is one we control and they do not.
...
data, it is effective dimensionality (participation ratio) and output calibration, not homology;
both are established correlates of forgetting (effective rank: \citet{Aghajanyan2021intrinsic}
and the continual-learning rank literature \citep{erankCF}; calibration: \citep{calibrationCF}),
and we claim neither as novel. The point for this paper is narrower and negative: the
\emph{topological} variable our ontology privileges is not the one that carries the signal.
...
capability. The same sweep clarifies a choice made earlier: the $\beta_1$ count does not separate
reasoning from hallucination at any threshold ($\beta_1^{R}\approx\beta_1^{H}$ throughout), which is why
we report mode separation as \emph{positional} (centroid/bottleneck), where it \emph{is}
...
shows the same direction; and \(\beta_2\) is empty under this protocol (the manifold's persistent
topology is confined to dimensions \(0\)--\(1\)).
...
The author thanks Gemini (Google DeepMind), DeepSeek, and the Hermes Agent framework for assistance during drafting, cross-referencing, and citation retrieval. All errors and claims remain the author's own.
