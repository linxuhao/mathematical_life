...
(partial $r = +0.18$, $p<.001$; AUC $\approx 0.63$ at matched token count), but an error-aligned
reading of six length-matched correct/incorrect pairs shows the $\beta_1$ gap is \emph{already present
before the first reasoning error} and does not change at error onset---it reflects which
\emph{problem} is being solved, not whether the reasoning is going right---and the powered
within-question test (same problem, multiple sampled solutions; \S\ref{sec:routing-test}) confirms
this: with question identity and completion length both controlled, the trajectory signal is at
chance (AUC $0.48$). The intra-trace topological signal thus decomposes exactly into the two
