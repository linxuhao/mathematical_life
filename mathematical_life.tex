...
\medskip
\noindent\textbf{The null holds \emph{within} traces, too---after two controls.} The result above is
static---one cloud per model. We also ran the dynamic, \emph{intra-trace} version, which most directly
matches the positive topological claims in the literature: for $469$ EOS-terminated reasoning
trajectories (regenerated with a $1024$-token budget so truncation cannot corrupt correctness labels;
accuracy $81\%$) we computed $\beta_1$ of each trajectory's token activations and asked whether it
predicts that trace's correctness. Naively it appears to, in the \emph{inverted} direction (AUC
$0.41$)---but the trajectory $\beta_1$ is in effect a token counter ($r = 0.95$ with trajectory
length), and incorrect traces simply run longer ($436$ vs.\ $375$ tokens on average). Controlling
length then exposes a second confound rather than a signal: a length-controlled residual remains
(partial $r = +0.18$, $p<.001$; AUC $\approx 0.63$ at matched token count), but two further checks
dissolve it. An error-aligned reading of six length-matched correct/incorrect pairs shows the
$\beta_1$ gap is \emph{already present before the first reasoning error} and does not change at
error onset---so the residual is not a \emph{consequence} of reasoning going wrong. That leaves two
candidate sources: the identity of the problem being solved, or an attempt-level early-warning
signal carried by the hidden state. The powered within-question test (same problem, multiple
sampled solutions; \S\ref{sec:routing-test}) adjudicates between them: an early-warning signal
would survive question centering, but with question identity and completion length both controlled
the trajectory-geometry signal is at chance (AUC $0.48$)---pointing to problem identity as the
dominant source. Because that test instruments trajectory geometry through route features rather
than cumulative $\beta_1$ itself, and because a post-release audit found the batch behind it
truncation-contaminated (footnote, \S\ref{sec:routing-test}), we pre-registered and ran a clean,
direct version: $200$ fresh questions at a doubled ($2048$-token) budget, $105$ of them mixed,
$755$ uncapped trajectories, with the verdict rule fixed in the repository \emph{before} any data
existed (\texttt{sameq\_b1\_*}). The result \emph{excludes} the precursor reading for $\beta_1$:
the within-question-centered AUC of filtered cumulative $\beta_1$ is $0.46/0.48/0.54$ at
$k=128/192/256$, below the pre-registered $0.55$ bar across the entire pre-error window
(per-question Wilcoxon all $p \geq 0.16$; exploratory $k=384$: $0.52$; full-trajectory $0.55$).
The dissociation is instructive: on the same clean labels the \emph{endpoint-activation} probe
survives (AUC $0.59$), so a faint attempt-level signal does exist in the raw hidden state---but
the topological summary does not carry it. Even where ``thinking wrong before the visible error''
is faintly decodable, $\beta_1$ is not the instrument that decodes it. The intra-trace topological signal thus decomposes exactly into the two
confounds this paper keeps finding---length, then difficulty/problem identity---with nothing left
over. (An earlier $1{,}319$-trajectory pilot of this test, generated under a $256$-token cap, is
excluded: $94\%$ of its trajectories were truncated, so its ``correctness'' labels largely measured
fitting within the budget; we retain it only as the motivation for the clean regeneration.)\footnote{Three further supplementary probes, released with the code, corroborate this section's nulls and none reverses them. (i) An \emph{aha-moment} test asks whether the \emph{cumulative} $\beta_1(k)$ curve of a trajectory shows a localized jump marking the moment reasoning goes right or wrong; it does not (the largest single-step jump is statistically indistinguishable between correct and incorrect traces, $p = 0.16$; \texttt{aha\_curves.json}). (ii) A \emph{thermometer} validity test checks whether the topological gauges ($\beta_1$, survival rate, the structural index) track out-of-distribution (SVAMP, TruthfulQA) transfer across the released checkpoint series better than chance once training step is partialled out; they do not (\texttt{thermometer\_analysis.py}). (iii) A TruthfulQA diagnostic (\texttt{tqa\_diagnostic.py}) shows that an apparent benchmark drop is largely a letter-position output bias rather than a topological effect. We report these as additional robustness nulls, not headline results.}
...
established. The nearest
analog of their object within our framework is the intra-trace test reported just above---nearest,
not identical: our points are token-level \emph{internal activations}, theirs are step-level
\emph{external sentence embeddings}, so the comparison is two removes from theirs (granularity and
representation), and our numbers do not transfer to their setup. With that caveat, the pattern we
find is cautionary for any per-trace topological feature set: trajectory $\beta_1$ covaried with
length far more strongly than anything their setup has been checked against ($r=0.95$), the naive
signal inverted under a length control, and the length-controlled residual was then fully absorbed
by question identity. Whether their step-level features behave the same way under those two controls
is precisely the unexamined question above.
...
the length-and-identity covariation which fully explained the analogous signal on \emph{our}
substrate (token-level, two removes from theirs) remains unexamined on theirs.
...
$\mathrm{AUC}=0.77$, and once it is controlled the route-efficiency signal vanishes ($\mathrm{AUC}=0.48$).\footnote{A
post-release audit of this same-question batch found it was generated under a $1024$-token budget
which $59\%$ of its mixed-question completions exhausted; capped completions' correctness labels
come from a last-number fallback and are partly truncation-driven. The length-controlled null is
partially protected---truncation is itself a length artifact, and completion length is exactly the
controlled variable---but label noise attenuates all AUCs toward chance, so the $0.48$, $0.69$,
$0.77$, and the endpoint-probe $0.58$ of \S\ref{sec:empirical-p1} should be read with that caveat.
A clean regeneration at a doubled budget, with the analysis and verdict rule pre-registered in the
repository \emph{before} any data existed (\texttt{sameq\_b1\_gen/extract/analyze.py}), has since
been run ($200$ questions, $105$ mixed, $755$ uncapped trajectories; capped rate falls from
$59\%$ to $43\%$ of mixed-question completions and capped items are excluded). Outcome: the
within-question cumulative-$\beta_1$ verdict is \emph{excluded} (AUC $0.46$--$0.54$, below the
pre-registered $0.55$ bar; \S\ref{sec:empirical-p1}), while the endpoint probe \emph{survives}
clean labels (AUC $0.59$ vs.\ the contaminated $0.58$)---so the weak endpoint signal was real,
not a truncation artifact. The route-feature numbers themselves were not recomputed; the clean
trajectory-level null is consistent with their conclusion.}
