...
dominant source. We flag the residual honestly: that test instruments trajectory geometry through
route features rather than through cumulative $\beta_1$ itself, whose within-question version we
have not run, and an endpoint-activation probe \emph{does} retain a weak within-question signal
(AUC $0.58$); moreover both within-question numbers come from a batch with a high truncation rate
whose labels are partly budget artifacts (see the audit footnote in \S\ref{sec:routing-test})---so
a small precursor component (the hidden state ``thinking wrong'' before the visible error) is
disfavored by everything we measured, but not excluded; a pre-registered clean re-test accompanies
the repository. The intra-trace topological signal thus decomposes exactly into the two
...
repository \emph{before} any data existed (\texttt{sameq\_b1\_gen/extract/analyze.py}),
re-adjudicates these numbers together with the within-question cumulative-$\beta_1$ test; its
results ship with the repository regardless of outcome.}
